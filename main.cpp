...
struct KekStruct {
    ...
    KekStruct(KekStruct&& kek) noexcept :
        ival(kek.ival), dval(kek.dval), sval(kek.sval) {
...
int main() {
    ...
    // bruteForceFind()
    assert(intMap.bruteForceFind(555)->second == 655);
    assert(doubleMap.bruteForceFind(13.5)->second == 113.5);
    assert(stringMap.bruteForceFind("key600")->second == "value600");
    ...
    // other functions
    std::cout << "totalBucketCount: " << doubleMap.totalBucketCount() << "\n";
    std::cout << "usedBucketCount: " << doubleMap.usedBucketCount() << "\n";
